...
function CremationForm(props){
  const { /*user,*/ cremationCert, deceased } = props
  ...
  const ref = React.createRef();
    ...
    dispatch(Actions.printCremationCertificate(match.params.id))
	...
	}, [cremationCert, deceased, setForm])


	console.log(match, "match")		
	console.log(form, "form")		
	console.log(cremationCert, "cremationCert")		
				...
				<div className='flex flex-col flex-wrap mt-0 mb-24 relative'>
					...
					<div className='w-9/12 mx-auto overflow-hidden sm:rounded-lg' ref={ref}>
						...
						<div className='border-t border-gray-200'>
							...
							<div className='bg-white flex flex-col px-1 py-8 sm:px-0'>
							...
							<div className='bg-gray-100 flex flex-col px-1 py-8 sm:px-0'>
								<span className='text-sm text-center font-bold text-gray-600 mb-8'>
									Age
								</span>
								<TextField 
							...
							<div className='bg-white grid grid-cols-3 gap-x-3 px-1 py-2 sm:px-0 mb-8'>
							...
							<div className='bg-white flex flex-col px-1 py-1 sm:px-0 my-24'>
								<span className='text-sm text-center font-bold text-gray-600'>
									has been cremated and in good sanitary condition
								</span>
							</div>
							<div className='bg-gray-100 grid grid-cols-3 gap-x-3 px-1 py-16 items-center text-center sm:px-0'>
								<div>
									<TextField 
										name="signature" 
...
const mapStateToProps = ({deceasedApp, auth}) => {
  ...
  return {
    ...
    cremationCert: deceased.cremationCert,
    deceased: deceased.deceased,
    user: auth.user.data,
  }
}

export default withReducer("deceasedApp", reducer)(withRouter(connect(mapStateToProps)(CremationForm)));