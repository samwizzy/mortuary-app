...
function VoucherList(props) {
  const { voucher, user, branches } = props
      ...
      <FuseAnimate delay={100}>
        <div className='flex flex-col flex-wrap mt-0 mb-24 relative'>
          <div className="absolute right-0 top-0 bg-orange-lighter">
            <Pdf targetRef={printRef} filename={`${voucher?.familyName}.pdf`} options={options} x={.4} y={.4} scale={0.92}>
              {({ toPdf }) => 
          ...
          <div className='w-9/12 mx-auto bg-white overflow-hidden sm:rounded-lg' ref={printRef}>
              ...
              <div>
                <dl className="space-y-16 text-right text-xs">
                  ...
                  <div className="text-gray-600">
                    <dt>{moment(voucher?.date).format("dddd, MMMM Do, YYYY")}</dt>
                  </div>
                </dl>
              </div>
            </div>
            ...
            <div className="text-center">
              <h2 className="uppercase text-lg italic text-gray-900">Funeral Voucher</h2>
            </div>
